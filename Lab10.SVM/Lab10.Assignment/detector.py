...
class Detector:
    ...
    def detectVideo(self, video_capture=None, num_frames=9, threshold=120,
        ...
        @param output_filename (str): Path for the output video.
        """
        ...
        if write:
            _, extension = os.path.splitext(output_filename.lower())
            fourcc = cv2.VideoWriter_fourcc(*("mp4v" if extension == ".mp4" else "MJPG"))
            writer = cv2.VideoWriter(output_filename, fourcc, write_fps, (w, h))
